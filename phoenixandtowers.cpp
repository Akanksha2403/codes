...
ll func()
{
    ...
    {
        new_int_1(num);
        vec.push_back(mp(num, i));
    }

    sort(all(vec));
    reverse(all(vec));
    set<pair<ll, ll>> sumvec;
    for (ll i = 0; i < m; i++)
    {
        sumvec.insert(mp(0, i));
    }
    ...
    for (ll i = 0; i < n; i++)
    {
        ll num = sumvec.begin()->first;
        ll index = sumvec.begin()->second;
        answer[vec[i].second] = index;
        num += vec[i].first;
        sumvec.erase(sumvec.begin());
        sumvec.insert(mp(num, index));
    }
    ll diff = prev(sumvec.end())->first - sumvec.begin()->first;
    if (diff <= k)
    {
        cout << "YES" << endl;
        for (auto i : answer)
