...
// ll dp[1000006] = {};
vector<pair<ll, ll>> dp (1000006, mp(-1, -1));
ll func(vi &vec, ll pos, ll k)
{
    if(dp[k].first != -1 && dp[k].second <= pos)
    {
        return dp[k].first;
    }
    if (k == 0)
    ...
    for (ll i = pos; i >= 0; i--)
    {
        ...
        ll ans = func(vec, i, k - vec[i]);
        if (ans != 0 && highestindex < i)
        {
            highestindex = i;
        }
        sum += ans;
...
int main()
{
    ...
    cout << func(vec, vec.size()-1, k) << endl; 
}