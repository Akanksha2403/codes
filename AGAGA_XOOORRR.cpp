...
ll func()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        totalxor = vec[i] ^ totalxor;
    }
    ll prevelexor = vec[0];
    if (vec[0] == totalxor ^ vec[0])
    {
        return true;
    }
    for (ll i = 1; i <= n - 1; i++)
    {
        // prevele, searched ele, last ele
        ll lastelexor = totalxor ^ (vec[i] ^ prevelexor);
        if (prevelexor == vec[i] && vec[i] == lastelexor)
        {
            return true;
        }
    }
