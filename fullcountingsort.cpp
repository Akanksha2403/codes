...
{
    new_int_1(n);
    map<ll, vector<pair<string, bool>>> ans;
    for (ll i = 0; i < n; i++)
    {
        ll num;
        string s;
        ...
        if (i < n / 2)
        {
            ...
            {
                vector<pair<string, bool>> contain{mp(s, 1)};
                ans[num] = contain;
            }
            else
            {
                ans[num].push_back(mp(s, 1));
            }
        }
        else
        {
            if (ans.find(num) == ans.end())
            {
                vector<pair<string, bool>> contain{mp(s, 0)};
                ans[num] = contain;
            }
            else
            {
                ans[num].push_back(mp(s, 0));
            }
        }
    }
    for (ll i = 0; i < 100; i++)
