...
ll func()
{
    ...
    com = com / ((ld)totalstars);
    ll position = round(com);

    ll numofstars = 0;
    ll distancetravelled = 0;

    for (ll i = position, j = position; i < n; i++)
    {
        if (s[i] == '.')
            continue;

        distancetravelled += abs(j - i);
        j++;
    }

    for (ll i = position, j = position; i >= 0; i--)
    {
        if (s[i] == '.')
            continue;

        distancetravelled += abs(i - j);
        j--;
    }
