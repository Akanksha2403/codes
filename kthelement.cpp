...
ld denominator(vi &elements, ll n)
{
    if (n == 0)
    {
        return 0;
    ...
    for (ll i = n - 1; i >= 0; i--)
    {
        ld lastelement = elements[i];
        sum += 1 / lastelement - (1 / lastelement) * (denominator(elements, i));
    }
    return sum;
}
ll func()
{
    ...
    ld deno = denominator(vec, n);
    ld approximate = (ld)k / deno;
    db(approximate);
    ...
    {
        // output();
        ll factor = ((ll)approximate) / (vec[i]);
        if (factor * vec[i] == approximate)
        {
            cout << factor * vec[i] << endl;
            // output();
            return 0;
        }
        // cout << factor*n << endl;
        if (min > vec[i] * (factor + 1))
        {
            min = vec[i] * (factor + 1);
        }
    }
